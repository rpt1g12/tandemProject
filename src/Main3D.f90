...
!===== DOMAIN DECOMPOSITION & BOUNDARY INFORMATION
 ...
 do mm=0,mbk
    ...
    cgrid='misc/grid'//cnzone//'.dat'; crestart='misc/restart'//cnzone//'.dat'
...
!===== GRID INPUT & CALCULATION OF GRID METRICS
 ...
 do j=0,let; jp=j*(lxio+1)
    ...
    open(9,file=cgrid,access='direct',form='unformatted',recl=3*nrecd)
    lp=lpos(myid)
...
!----- VISCOUS SHEAR STRESSES & HEAT FLUXES
 ...
 if(nviscous==1) then
    ...
    txx(:)=rr(:,1)*(two*txx(:)-de(:,5))
    tyy(:)=rr(:,1)*(two*tyy(:)-de(:,5))
    tzz(:)=rr(:,1)*(two*tzz(:)-de(:,5))
    txy(:)=rr(:,1)*(txy(:)+hzz(:))
    tyz(:)=rr(:,1)*(tyz(:)+hxx(:))
    tzx(:)=rr(:,1)*(tzx(:)+hyy(:))
...
!----- WALL TEMPERATURE & VELOCITY CONDITION
 ...
 do k=0,ijk(3,nn); do j=0,ijk(2,nn); l=indx3(i,j,k,nn); lp=lp+1; ra2=one-rpex(lp); ra3=one-ra1*ra2
...
!----- JUNCTION & INTERFACE AVERAGING
 ...
 do k=0,ijk(3,nn); kp=k*(ijk(2,nn)+1)
 do j=0,ijk(2,nn); jk=kp+j; l=indx3(i,j,k,nn)
    rr(l,1)=rr(l,1)+one; rr(l,2)=one/rr(l,1); qa(l,:)=rr(l,2)*((rr(l,1)-one)*qa(l,:)+drvb(jk,:,ip))
 end do
 end do
 end if
 if(abs(nextgcic-1)+abs((np-20)*(np-25))==0) then
 do k=0,ijk(3,nn); kp=k*(ijk(2,nn)+1)
 do j=0,ijk(2,nn); jk=kp+j; l=indx3(i,j,k,nn); lp=lp+1; ra0=rpex(lp); ra1=one-ra0
    rr(l,1)=rr(l,1)+ra0; rr(l,2)=one/rr(l,1); qa(l,:)=ra0*rr(l,2)*((rr(l,1)-one)*qa(l,:)+drvb(jk,:,ip))+ra1*qa(l,:)
 end do
 end do
 end if
 end do
