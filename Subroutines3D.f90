...
!===== SUBROUTINE FOR MPI IMPLEMENTATION
 ...
 subroutine mpigo(nt,itag,ng)

 integer,intent(in) :: nt,itag,ng
 ...
 do ip=0,1; iq=1-ip; is=ip*ijk(1,nn); ie=1-2*ip; np=(nbc(ip,nn)-40)/5; ii=abs(np); npg=max(np,0)*ng
 if(ndf(ip,nn)==1) then
 ...
 do ip=0,1; is=ip*ijk(1,nn); np=(nbc(ip,nn)-40)/5
 if(np==1) then
 do k=0,ijk(3,nn); kp=k*(ijk(2,nn)+1)
 do j=0,ijk(2,nn); jk=kp+j; l=indx3(is,j,k,nn)
    recv(jk,0,ip)=recv(jk,0,ip)+rr(l,nn)*pbcot(0,nt)
    recv(jk,1,ip)=recv(jk,1,ip)+rr(l,nn)*pbcot(1,nt)
    recv(jk,2,ip)=recv(jk,2,ip)+rr(l,nn)
 end do
 end do
 end if
 end do
...
!===== SUBROUTINE FOR COMPACT FILTERING
 ...
 subroutine filte(nn,m)

 integer,intent(in) :: nn,m

    nt=1; ns=ndf(0,nn); ne=ndf(1,nn)
 ...
 if(nfbco==0) then
    sb(is)=0; sb(is+1)=0
    sb(is+2)=dot_product(fbc(:),(/sa(is),sa(is+1),sa(is+3),sa(is+4),sa(is+5)/)-sa(is+2))
 else
    res=sum((/fex1,fex2,fex3/)*(sa(is+(/1,2,3/)*nfbcn)-sa(is))); rv(:)=sa(is)-(/1,2,3/)*res
    res=2*sa(is); sb(is)=fa*(rv(1)+sa(is+1)-res)+fb*(rv(2)+sa(is+2)-res)+fc*(rv(3)+sa(is+3)-res)
    res=2*sa(is+1); sb(is+1)=fa*(sa(is)+sa(is+2)-res)+fb*(rv(1)+sa(is+3)-res)+fc*(rv(2)+sa(is+4)-res)
    res=2*sa(is+2); sb(is+2)=fa*(sa(is+1)+sa(is+3)-res)+fb*(sa(is)+sa(is+4)-res)+fc*(rv(1)+sa(is+5)-res)
 end if
 case(1)
    sb(is)=sum(pbci(0:lmf,0,nt)*sa(is:is+lmf))+recv(jk,0,0)
    sb(is+1)=sum(pbci(0:lmf,1,nt)*sa(is:is+lmf))+recv(jk,1,0)
    res=2*sa(is+2); sb(is+2)=fa*(sa(is+1)+sa(is+3)-res)+fb*(sa(is)+sa(is+4)-res)+fc*(recv(jk,2,0)+sa(is+5)-res)
 end select
 do i=is+3,ie-3
 ...
 if(nfbco==0) then
    sb(ie)=0; sb(ie-1)=0
    sb(ie-2)=dot_product(fbc(:),(/sa(ie),sa(ie-1),sa(ie-3),sa(ie-4),sa(ie-5)/)-sa(ie-2))
 else
    res=sum((/fex1,fex2,fex3/)*(sa(ie)-sa(ie-(/1,2,3/)*nfbcn))); rv(:)=sa(ie)+(/1,2,3/)*res
    res=2*sa(ie); sb(ie)=fa*(sa(ie-1)+rv(1)-res)+fb*(sa(ie-2)+rv(2)-res)+fc*(sa(ie-3)+rv(3)-res)
    res=2*sa(ie-1); sb(ie-1)=fa*(sa(ie-2)+sa(ie)-res)+fb*(sa(ie-3)+rv(1)-res)+fc*(sa(ie-4)+rv(2)-res)
    res=2*sa(ie-2); sb(ie-2)=fa*(sa(ie-3)+sa(ie-1)-res)+fb*(sa(ie-4)+sa(ie)-res)+fc*(sa(ie-5)+rv(1)-res)
 end if
 case(1)
    sb(ie)=sum(pbci(0:lmf,0,nt)*sa(ie:ie-lmf:-1))+recv(jk,0,1)
    sb(ie-1)=sum(pbci(0:lmf,1,nt)*sa(ie:ie-lmf:-1))+recv(jk,1,1)
    res=2*sa(ie-2); sb(ie-2)=fa*(sa(ie-3)+sa(ie-1)-res)+fb*(sa(ie-4)+sa(ie)-res)+fc*(sa(ie-5)+recv(jk,2,1)-res)
 end select
    sa(is)=sb(is)
